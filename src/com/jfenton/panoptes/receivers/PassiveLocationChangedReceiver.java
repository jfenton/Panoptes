...
package com.jfenton.panoptes.receivers;

import com.jfenton.panoptes.PanoptesConstants;
import com.jfenton.panoptes.services.EclairPlacesUpdateService;
import com.jfenton.panoptes.services.PanoptesUpdateService;
import com.jfenton.panoptes.utils.LegacyLastLocationFinder;
...
public class PassiveLocationChangedReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (intent.hasExtra(key)) {
			...
			location = lastLocationFinder.getLastBestLocation(PanoptesConstants.MAX_DISTANCE, System.currentTimeMillis() - PanoptesConstants.MAX_TIME);
			SharedPreferences prefs = context.getSharedPreferences(PanoptesConstants.SHARED_PREFERENCE_FILE, Context.MODE_PRIVATE);
			...
			// transfers).
			if ((lastTime > System.currentTimeMillis() - PanoptesConstants.MAX_TIME) || (lastLocation.distanceTo(location) < PanoptesConstants.MAX_DISTANCE))
				location = null;
		}
		...
		if (location != null) {
			...
			Intent updateServiceIntent = new Intent(context, PanoptesConstants.SUPPORTS_ECLAIR ? EclairPlacesUpdateService.class : PanoptesUpdateService.class);
			updateServiceIntent.putExtra(PanoptesConstants.EXTRA_KEY_LOCATION, location);
			updateServiceIntent.putExtra(PanoptesConstants.EXTRA_KEY_RADIUS, PanoptesConstants.DEFAULT_RADIUS);
			updateServiceIntent.putExtra(PanoptesConstants.EXTRA_KEY_FORCEREFRESH, false);
