...
public class PlaceCheckinService extends IntentService {
  ...
  public void onCreate() {
    ...
    sharedPreferences = getSharedPreferences(PanoptesConstants.SHARED_PREFERENCE_FILE, Context.MODE_PRIVATE);
    sharedPreferencesEditor = sharedPreferences.edit();
    sharedPreferenceSaver = PlatformSpecificImplementationFactory.getSharedPreferenceSaver(this);
    
    Intent retryIntent = new Intent(PanoptesConstants.RETRY_QUEUED_CHECKINS_ACTION);
  ...
  protected void onHandleIntent(Intent intent) {
    ...
    String reference = intent.getStringExtra(PanoptesConstants.EXTRA_KEY_REFERENCE);
    String id = intent.getStringExtra(PanoptesConstants.EXTRA_KEY_ID);
    long timeStamp = intent.getLongExtra(PanoptesConstants.EXTRA_KEY_TIME_STAMP, 0);
  	...
  	boolean inBackground = sharedPreferences.getBoolean(PanoptesConstants.EXTRA_KEY_IN_BACKGROUND, true);
...
//        if (response.getStatusLine().getReasonPhrase().equals(PanoptesConstants.PLACES_CHECKIN_OK_STATUS)) {
          ...
          if (timeStamp > lastCheckin) {   
            ...
            sharedPreferencesEditor.putString(PanoptesConstants.SP_KEY_LAST_CHECKIN_ID, id);
            sharedPreferenceSaver.savePreferences(sharedPreferencesEditor, false);
            Intent intent = new Intent(PanoptesConstants.NEW_CHECKIN_ACTION);
            intent.putExtra(PanoptesConstants.EXTRA_KEY_ID, id);
